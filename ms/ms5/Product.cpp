		...
		if (isClear()) {
			if (!isEmpty()) {
				if (writeMode == write_condensed) {
					out << productType << "," << productSku << "," << productName << "," << productUnitDesc << "," << std::fixed << std::setprecision(2) << productUnitPrice << "," << (int)productTaxable << "," << qtyAvailable() << "," << qtyNeeded();
				}
					...
					out << std::left << std::setw(10) << productUnitDesc << " | "
						<< std::right << std::right << std::fixed << std::setw(7) << productUnitPrice << " | ";
